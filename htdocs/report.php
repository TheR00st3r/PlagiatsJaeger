		...
		foreach ($array as $a) {
			...
			if ($a['type'] != 0) {
				...
				if ($a['rtSourcedID']) {
					...
					$source = '<div class="rtSourceText"><a target="_blank" href="' . urldecode($a['rtSourceLink']) . '" title="zu ' . $a['rtSimilarity'] . ' % von '.urldecode($a['rtSourceLink']) .'" />' . nl2br($a['rtSourceText']) . '</a></div>';
				}
